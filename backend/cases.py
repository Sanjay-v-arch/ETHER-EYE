...
class CaseManager:
    ...
    def __init__(self, neo4j_driver, blockchain_connector: BlockchainConnector):
    ...
    async def validate_tx_hash(self, tx_hash: str, chain: str) -> bool:
        """Validate transaction hash using BlockchainConnector."""
        async with aiohttp.ClientSession() as session:
            tx_data = await self.connector.get_transaction_data(session, tx_hash, chain)
            return bool(tx_data)
    ...
    async def create_case(self, name: str, investigator: str, status: str = "open") -> Optional[int]:
        ...
        try:
            ...
            case_id = await asyncio.to_thread(run_query)
    ...
    async def associate_transaction(self, case_id: int, tx_hash: str, chain: str, user: str) -> bool:
        """Associate a transaction with a case in Neo4j."""
        if not isinstance(case_id, int) or case_id < 0 or not await self.validate_tx_hash(tx_hash, chain) or not user.strip():
            logger.error(f"Invalid parameters: case_id={case_id}, tx_hash={tx_hash}, chain={chain}, user={user}")
            return False
        ...
        try:
            ...
            if not await asyncio.to_thread(run_query):
    ...
    async def get_case_details(self, case_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            record = await asyncio.to_thread(run_query)
            if not record:
                logger.error(f"Case {case_id} not found")
                return None
...
if __name__ == "__main__":
    ...
    manager = CaseManager(neo4j_driver, connector)
    async def test():
        case_id = await manager.create_case("Test Case", "Alice")
        if case_id:
            await manager.associate_transaction(case_id, "0x4e3a3754410177e6937ef1d0084000883f919978", "ethereum", "Alice")
